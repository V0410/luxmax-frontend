export default defineNuxtPlugin(() => {
  return {
    provide: {
      dateToShow: (date) => {
        const now = Date.now();
        let dateTimestamp = new Date(+date * 1000);
        //is on IOS
        if (Number.isNaN(dateTimestamp.getMonth())) {
          let arr = date.split(/[- :]/);
        ...
        //is on IOS
        const isInSameDay = (now, dateTimestamp) => {
        ...
        const isYesterday = (now, dateTimestamp) => {
          const nowDate = new Date(now);
          const date = new Date(dateTimestamp);
          return (
            nowDate.getDate() === date.getDate() + 1 &&
            nowDate.getMonth() === date.getMonth() &&
            nowDate.getFullYear() === date.getFullYear()
          );
        };
        if (isInSameDay(now, dateTimestamp)) {
          const { t } = useI18n();
          return t("Today");
        } else if (isYesterday(now, dateTimestamp)) {
          const { t } = useI18n();
          return t("Yesterday");
        } else {
      ...
      formatItems: (itemsArray) => {
        ...
        return itemsArray.map((el) => {
          ...
          return {
            id: el.id,
            img: el?.images[0] ? el?.images[0] : null,
            imgs: el?.images,
            brand: el?.brand || el?.brand?.name,
            name: el.category.name || "",
            price: el?.price
              ? Math.floor(
                  (+el.price - (+el.price * (+el.off || 0)) / 100) * 100 || 0
                ) / 100
              : 0,
            priceBeforeDiscount: Math.floor(+el.price * 100 || 0) / 100 || 0,
            discountPercentage: Math.floor(+el.off * 100 || 0) / 100 || 0,
            color: el.color || null,
            state: el?.state,
            isPopular: el.isPopular || false,
            item: {
              img: el.images[0] ? el.images[0] : null,
              text: el.category.name || "",
              desc: el?.brand || el?.brand?.name,
            },
            buyerInfo: {
              text: "-",
            },
      ...
      detailNameMapper: (name) => {
        const { t } = useI18n();
        const mapObject = {
          brand_name: t("Brand"),
          category: t("Category"),
          sub_category: t("Sub category"),
          countryOfOrigin: t("Country of origin"),
          size: t("Size"),
          yearOfManufacturing: t("Year of manufacturing"),
          exteriorColor: t("Exterior color"),
          exteriorMaterial: t("Exterior material"),
          interiorColor: t("Interior color"),
          interiorMaterial: t("Interior material"),
          handDropColor: t("HandDrop color"),
          handDropMaterial: t("HandDrop material"),
          solesColor: t("Soles color"),
          solesMaterial: t("Soles material"),
        };
        return mapObject[name] || name;
      },
      phoneRegExp: /^([1-9]{4}|0[1-9]{4}) ?\d{6}|^$/,
      UKPhoneRegExp: /^7\d{9}$/,
      postalCodeRegex:
